    ...
    public void initEvent() {
        ...
        ((Button) $(mainwindow,"myHead")).setOnAction(event -> {
            ...
            try{
                ...
                String Age = age + "";
                ((TextField) $(homepage,"birth")).setText(Age + "宀�");
                ...
                try {
                    ...
                    if(!resultSet.next()){
                        if(password.equals(rePassword)){
                            ...
                            database.exec("insert into user values(?,?,?,?,?,?,?,?,?,?,?)",account,nickname,name,password,birthday,sex,userdata.getHead(),"河南省郑州市","",phone,"background6");
                    ...
                    if(resultSet.next()){
                        if(resultSet.getString("password").equals(password)){
                            ...
                            if(set.next()){
                                ...
                                /*设置聊天助手*/
                                try {
                                    mainwindow.addFriend("助手Siri","head9","助手Siri","快来找我聊天吧!");
                                    ((Button) $(Controller.ch,"friendMore")).setText("助手Siri");
                                }catch (IOException e){
                                    e.printStackTrace();
                                }
                                messageData.msg.add(new Vector<>());
                                messageData.account.add("助手Siri");
                                messageData.msgTip.put("助手Siri",0);
                                /*把所有好友添加到列表*/
    ...
    public void sendMsg(){
        ((Button) $(ch,"send")).setOnAction(event -> {
            ...
            try{
                ...
                if(resultSet.next() && !remark.equals("助手Siri")){
            ...
            if(!text.equals("")){
                try{
                    if (Y_account.equals("助手Siri")){
                        try {
                            String xiong = (new TuLing().robot(text)).toString();
                            ch.addRight(userdata.getHead(),text); //我的信息
                        ...
                        if(resultSet.next()){
                            ...
                            try{
                                ...
                                if(i != -1){
                                    messageData.msg.get(i).add(userdata.getAccount() + " " + text); //娣诲姞鍒版秷鎭泦
                                }
                                ((TextField) $(ch,"input")).clear(); //添加到消息集
                            }catch (IOException e){
                                alert.setText("您断开了连接!");
                                alert.exec();
                                e.printStackTrace();
                            }
                        }else {
